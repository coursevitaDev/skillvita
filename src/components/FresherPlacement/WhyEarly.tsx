...
export default function WhyEarly() {
  return (
    <section className="py-24 bg-zinc-900 relative overflow-hidden">
      ...
      <motion.div animate={{ scale: [1, 1.2, 1], rotate: [0, 90, 0] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute top-0 right-0 w-64 md:w-96 h-64 md:h-96 bg-brand-600 rounded-full blur-[80px] md:blur-[120px] opacity-10 pointer-events-none"></motion.div>
      <div className="absolute bottom-0 left-0 w-64 md:w-96 h-64 md:h-96 bg-brand-800 rounded-full blur-[80px] md:blur-[120px] opacity-10 pointer-events-none"></div>
      <div className="container mx-auto px-4 md:px-6 lg:px-12 relative z-10 text-center">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={scaleUp}>
          ...
          <motion.button whileHover={{ scale: 1.05, backgroundColor: "#22c55e", color: "#000", boxShadow: "0 0 30px rgba(34,197,94,0.6)" }} whileTap={{ scale: 0.95 }} className="px-8 md:px-12 py-4 md:py-5 bg-[#22c55e] text-black font-bold rounded-full text-lg md:text-xl shadow-[0_0_20px_rgba(34,197,94,0.4)] transition-all">Start Learning</motion.button>
        </motion.div>
      </div>
    </section>
  );
